...
class SessionItem : public QObject {
  Q_OBJECT
  Q_PROPERTY(QQmlListProperty<DeviceItem> devices READ getDevices NOTIFY
                 devicesChanged)
  ...
  Q_PROPERTY(
      unsigned sampleCount MEMBER m_sample_count NOTIFY sampleCountChanged)
  ...
  Q_PROPERTY(
      int availableDevices READ getAvailableDevices NOTIFY devicesChanged)
...
/// DeviceItem abstracts over a LibSMU Device exposing relevant parameters to
/// QML
class DeviceItem : public QObject {
 ...
 public:
  ...
  Q_INVOKABLE int ctrl_transfer(int x, int y, int z) {
    return m_device->ctrl_transfer(0x40, x, y, z, 0, 0, 100);
  }
...
class ChannelItem : public QObject {
 ...
 public:
  ...
  QString getLabel() const {
    return QString(m_device->channel_info(m_index)->label);
  }
...
class SignalItem : public QObject {
 ...
 public:
  ...
  double getMeasurement() { return m_measurement; }
  double getPeak() { return m_peak_to_peak; }
