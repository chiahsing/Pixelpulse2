...
void registerTypes() {
  qmlRegisterAnonymousType<SessionItem>("", 1);
  qmlRegisterAnonymousType<DeviceItem>("", 1);
  qmlRegisterAnonymousType<ChannelItem>("", 1);
  qmlRegisterAnonymousType<SignalItem>("", 1);
  qmlRegisterAnonymousType<SrcItem>("", 1);
...
SessionItem::SessionItem()
    : m_session(new Session),
      ...
      m_logging(0) {
  connect(this, &SessionItem::finished, this, &SessionItem::onFinished,
          Qt::QueuedConnection);
  connect(this, &SessionItem::attached, this, &SessionItem::onAttached,
          Qt::QueuedConnection);
  connect(this, &SessionItem::detached, this, &SessionItem::onDetached,
          Qt::QueuedConnection);

  connect(this, &SessionItem::sampleCountChanged, this,
          &SessionItem::onSampleCountChanged);
  connect(this, &SessionItem::loggingChanged, this,
          &SessionItem::onLoggingChanged);
  connect(this, &SessionItem::sampleTimeChanged, this,
          &SessionItem::onSampleTimeChanged);
...
void SessionItem::onSampleCountChanged() { restart(); }
...
void SessionItem::handleDownloadedFirmware() {
  FileIO f;
  f.writeRawByFilename(getTmpPathForFirmware() + "/firmware.bin",
                       m_firmware_fd->downloadedData());
  emit firmwareDownloaded();
  disconnect(m_firmware_fd, SIGNAL(downloaded()), this,
             SLOT(handleDownloadedFirmware()));
}
...
void SessionItem::downloadFromUrl(QString url) {
  ...
  connect(m_firmware_fd, SIGNAL(downloaded()), this,
          SLOT(handleDownloadedFirmware()));
}
...
void SessionItem::usb_handle_thread_method(SessionItem* session_item) {
  ...
  while (true) {
    ...
    std::vector<Device*> available_devices =
        session_item->m_session->m_available_devices;
    ...
    for (auto other_device : last_devices) {
      ...
      if (!found) {
        ...
        last_devices.erase(
            remove(last_devices.begin(), last_devices.end(), other_device),
            last_devices.end());
      }
    }
    ...
    for (auto device : available_devices) {
      ...
      if (!found) {
        ...
        available_devices.erase(
            remove(available_devices.begin(), available_devices.end(), device),
            available_devices.end());
      }
    }

    last_devices.insert(last_devices.end(), available_devices.begin(),
                        available_devices.end());
  }
}
...
DeviceItem::DeviceItem(SessionItem* parent, Device* dev)
    : QObject(parent), m_device(dev), m_samples_added(0) {
...
void ChannelItem::buildTxBuffer() {
  ...
  qDebug() << "v1:" << v1 << " |v2:" << v2 << " |period:" << period
...
SignalItem::SignalItem(ChannelItem* parent, int index, Signal* sig)
  ...
  Q_UNUSED(sig_info);
  connect(m_channel, &ChannelItem::modeChanged, this,
          &SignalItem::onParentModeChanged);
}
...
DataLogger::DataLogger(float sampleTime, QObject* parent) : QObject(parent) {
  ...
  if (sampleTime != -1 && !(fabs(sampleTime - 0.01) < 0.00001 ||
                            fabs(sampleTime - 0.1) < 0.00001)) {
    ...
    lastLog = startTime;
    fileStream << "Timestamp,Device serial,Min V ch A,Min A ch A,Min V ch "
                  "B,Min A ch B,Max V ch A,Max A ch A,Max V ch B,Max A ch "
                  "B,Avg V ch A,Avg A ch A,Avg V ch B,Avg A ch B\n";
    fileStream.flush();
  }
}
...
void DataLogger::updateMinimum(DeviceItem* deviceItem,
...
void DataLogger::updateMaximum(DeviceItem* deviceItem,
                               std::array<float, 4> samples) {
...
void DataLogger::updateSum(DeviceItem* deviceItem,
                           std::array<float, 4> samples) {
...
std::array<float, 4> DataLogger::computeAverage(DeviceItem* deviceItem) {
  return {sum[deviceItem][0] / dataCounter[deviceItem],
          sum[deviceItem][1] / dataCounter[deviceItem],
          sum[deviceItem][2] / dataCounter[deviceItem],
          sum[deviceItem][3] / dataCounter[deviceItem]};
}
...
void DataLogger::addData(DeviceItem* deviceItem, std::array<float, 4> samples) {
  ...
  std::chrono::duration<double> timeDiff =
      std::chrono::system_clock::now() - lastLog;
...
void DataLogger::addBulkData(DeviceItem* deviceItem,
                             std::vector<std::array<float, 4>> buff) {
...
void DataLogger::printData(DeviceItem* deviceItem) {
  ...
  fileStream << setprecision(3) << fixed << timeDiff.count() << ","
             << deviceSerial << "," << minimum[deviceItem][0] << ","
             << minimum[deviceItem][1] << "," << minimum[deviceItem][2] << ","
             << minimum[deviceItem][3] << "," << maximum[deviceItem][0] << ","
             << maximum[deviceItem][1] << "," << maximum[deviceItem][2] << ","
             << maximum[deviceItem][3] << "," << average[0] << "," << average[1]
             << "," << average[2] << "," << average[3] << '\n';
  fileStream.flush();
  m_logMutex.unlock();
}
...
void DataLogger::doAddBulkData(DeviceItem* deviceItem,
                               std::vector<std::array<float, 4>> buff) {
