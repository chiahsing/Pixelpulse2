...
ColumnLayout {
    ...
    function deviceManagerListFill() {
        ...
        for (var i = 0; i < session.devices.length; i++) {
            // The firmware might be so old that won't provide the version in the 'major.minor' format

            var device = session.devices[i];
            var updt_needed = false;
            if (device.FWVer.indexOf('.') === -1) {
                updt_needed = true;
                updateNeeded = true;
    ...
    function checkFWversion() {
        JSUtils.checkLatestFw(function(ver) {
            ...
            if (err === JSUtils.GIT_RATE_LIMIT_EXCEEDED)
                logOutput.appendMessage('Failed to get the latest firmware version. ' + 'Number of requests to GIT was exceeded. A new request will be possible in an hour.');
            else
                logOutput.appendMessage('Failed to get the latest firmware version. ' + 'Check your internet connection and then click the "Refresh" button.');
    ...
    Rectangle {
        ...
        ListModel {
            ...
            property variant states: {
                "ok": 'Succesfully updated. Disconnect device.',
                "error": 'Failed to load firmware.',
                "prog": 'In programming mode.',
                "fw_ok": 'Firmware is up to date.',
                "n/a": ''
            }
            property variant statesColor: {
                "ok": 'green',
                "error": 'red',
                "prog": 'blue',
                "fw_ok": 'white',
                ...
                Column {
                    ...
                    Item {
                        ...
                        Row {
                            ...
                            Text {
                                text: "Firmware Version: " + firmware_version
                                font.pointSize: 10
                                color: 'white'
                                anchors.verticalCenter: parent.verticalCenter
                            }
                    ...
                    Item {
                        ...
                        Text {
                            text: "Hardware Version: " + hardware_version
                            font.pointSize: 10
                            color: 'white'
                            anchors.verticalCenter: parent.verticalCenter
                        }
                    ...
                    Item {
                        ...
                        Row {
                            ...
                            Text {
                                text: devicesModel.states[status]
                                font.pointSize: 10
