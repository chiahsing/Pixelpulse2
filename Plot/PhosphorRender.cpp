...
void Shader::updateState(const RenderState& state, QSGMaterial* newMaterial,
                         QSGMaterial* oldMaterial) {
  ...
  program()->setUniformValue(m_id_matrix,
                             state.combinedMatrix() * m->transformation);
  ...
  // glPointSize(m->pointSize); // Commenting this out since it is not available
  // in QOpenGLFunctions (OpenGL ES 2.0) and apparently does not affect the
  // point size
}
...
QSGNode* PhosphorRender::updatePaintNode(QSGNode* oldNode,
                                         UpdatePaintNodeData*) {
  if (!m_ybuffer) {
    return 0;
  }
  ...
  if (!oldNode) {
    ...
    geometry =
        new QSGGeometry(QSGGeometry::defaultAttributes_Point2D(), n_points);
