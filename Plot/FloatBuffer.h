...
class FloatBuffer : public QObject {
 ...
 public:
  ...
  Q_INVOKABLE float get(unsigned i) { return m_data[wrapIndex(i)]; }
  ...
  void append_samples(const std::vector<std::array<float, 4>>& samples,
                      int signal_index) {
    ...
    if (m_length < m_start_test) m_length = m_start_test;

    emit dataChanged();
  }

  void append_samples_circular(const std::vector<std::array<float, 4>>& samples,
                               int signal_index) {
    int num_samples_to_add = samples.size();
    if (m_length > maxSize) {
      m_data.erase(m_data.begin(),
                   std::min(m_data.end(), m_data.begin() + num_samples_to_add));
      for (int i = 0; i < num_samples_to_add; i++) {
  ...
  void startSweep() {
    // When switching from continuous to repeated-sweep mode, stop acting like a
    // ring buffer
    if (m_start != 0) {
      m_start = 0;
      m_length = 0;
  ...
  double rms() {
    ...
    return sqrt(accumulate(tmp.begin(), tmp.end(), 0.0, square<long double>()) /
                tmp.size());
  }

  double peak_to_peak() {
    if (m_data.size() != 0) {
      return *std::max_element(data_begin(), m_data.end()) -
             *std::min_element(data_begin(), m_data.end());
    }
    return 0;
  }
  ...
  std::vector<float> dif_mean(double avg) {
    ...
    for (std::deque<float>::iterator it = data_begin(); it != m_data.end();
         ++it) {
 ...
 public slots:
  QObject* getObject() { return new FloatBuffer(); }
 ...
 private:
  ...
  double indexToTime(unsigned index) { return index * m_secondsPerSample; }

  /* A number of samples at the beginning of the buffer can be ignored by
   * the application. Use data_begin() and data_size() to access the rest of
   * data */
  std::deque<float>::iterator data_begin() {
    return (m_data.begin() + m_first_samples_ignored);
  }

  float data_size() { return (m_data.size() - m_first_samples_ignored); }
};
