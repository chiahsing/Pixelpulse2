...
class PhosphorRender : public QQuickItem {
  ...
  Q_PROPERTY(
      FloatBuffer* buffer READ buffer WRITE setBuffer NOTIFY yBufferChanged)
  Q_PROPERTY(
      FloatBuffer* xBuffer READ xBuffer WRITE setXBuffer NOTIFY xBufferChanged)
  ...
  Q_PROPERTY(double pointSize READ pointSize WRITE setPointSize NOTIFY
                 pointSizeChanged)
...
#define SETTER(PROP)            \
  ...
  void setBuffer(FloatBuffer* ybuffer) {
    if (m_ybuffer != ybuffer) {
      ...
      m_ybuffer = ybuffer;
      if (m_ybuffer) {
        QObject::connect(m_ybuffer, &FloatBuffer::dataChanged, this,
                         &PhosphorRender::update);
      }
  ...
  void setXBuffer(FloatBuffer* xbuffer) {
    ...
    m_xbuffer = xbuffer;
    if (m_xbuffer) {
      QObject::connect(m_xbuffer, &FloatBuffer::dataChanged, this,
                       &PhosphorRender::update);
    }
  ...
  signals : void xBufferChanged(FloatBuffer* b);
